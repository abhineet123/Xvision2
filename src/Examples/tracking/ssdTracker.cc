...
//   typedef XVRemoteWindowX<XV_RGB >    WIN;
  ...
  typedef XVSSD<XVImageRGB<XV_RGB>,
                XVPyramidStepper<XVSE2Stepper<XVImageRGB<XV_RGB> > > > SSD ;
  ...
  XVSE2State state;
  state.trans=roi; state.angle=0.0; state.scale=1.0;
  SSD ssd(state,tmpl);
  //ssd.initState( XVStatePair<XVSE2State, double>(state,1.0));
