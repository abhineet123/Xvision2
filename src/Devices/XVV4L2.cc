...
int XVV4L2<T>::open(const char *dev_name,const char *parm_string)
{
  ...
  struct v4l2_standard supp_vid_stds;
  memset(&supp_vid_stds,0,sizeof(struct v4l2_standard));
  unsigned int index=0;
  supp_vid_stds.index=index;
  while(1){
	  if(ioctl (fd, VIDIOC_ENUMSTD, &supp_vid_stds)){
		  perror ("ENUMSTD in capture_init");
		  throw 11;
		  break;
	  }else{
		  printf("Index %d successful\n", index);
		  printf("Standard name: %s\n", supp_vid_stds.name);
		  printf("Standard id: %lu\n", supp_vid_stds.id);
		  index++;
		  supp_vid_stds.index=index;
	  }
  }
